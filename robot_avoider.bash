...
do
  ...
  l_range=$(get_scan_left_ray_range)
  echo " left range: $l_range meters"
  ...
  r_range=$(get_scan_right_ray_range)
  echo " right range: $r_range meters"
...
if [[ "$front_left_free" == 0 && "$front_free" == 0 && "$front_right_free" == 0 ]]; then
     ...
     # if left scan range is more than right scan range
      # turn left
      if [[ $(echo "$l_range > $r_range" | bc -l) == "1" ]]; then
        echo "turning left..."
  ...
  # elif front left is not free and front is free and front right is not free
    ...
    # calculate time with fixed speed of 0.3 m/s
      time_to_move=$(echo "$dist_to_move / 0.300" | bc -l)
    # subtract 1 second for parameter setting delay
      ...
      echo "moving forward..."
      if [[ $(echo "$time_to_move > 0" | bc -l) == "1" ]]; then
        set_cmd_vel_linear 0.300
        sleep "$time_to_move"
        set_cmd_vel_linear 0.000
      else
        echo "Distance too small to move forward safely."
      fi
  ...
  # else
    ...
    # move forward for roughly (front range - threshold) meters
    elif [[ "$front_left_free" == 1 && "$front_free" == 1 && "$front_right_free" == 1 ]];then
      dist_to_move=$(echo "$ff_range - $threshold" | bc -l)
    # calculate time with fixed speed of 0.3 m/s
      time_to_move=$(echo "$dist_to_move / 0.300" | bc -l)
    # subtract 1 second for parameter setting delay
